<?php
	...
	require 'clases/alojamientos_restaurantes.cls.php';
	...
	$mensaje2 = '';
	$error = '';
	$insertaAlojamientos_restaurantes = '';
	...
	if(count($_POST) != 0){
		...
		if($parametros['ir_a'] != 'S' and $parametros['ir_a'] != 'V'){
			...
			if($parametros['botonSelector'] != 0){
				$botonAlojamientos_restaurantes = new clsAlojamientos_restaurantes($conexion, $filadesde, $usuario, $parametros['buscar_alojamiento']);
				$botonselec = $botonAlojamientos_restaurantes->Botones_selector($filadesde, $parametros['botonSelector']);
			...
			if($parametros['actuar'] == 'S'){
				...
				for ($num_fila = 0; $num_fila <= $Nfilas['LINEAS_MODIFICACION']-1; $num_fila++) {
					if($parametros['selec'.$num_fila] == 'S' || $parametros['borra'.$num_fila] == 'S'){
						if($parametros['selec'.$num_fila] == 'S'){
							...
							$mAlojamientos_restaurantes = new clsAlojamientos_restaurantes($conexion, $filadesde, $usuario, $parametros['buscar_alojamiento']);
							$modificaAlojamientos_restaurantes = $mAlojamientos_restaurantes->Modificar($parametros['id_alojamiento'.$num_fila], $parametros['nombre'.$num_fila],$parametros['descripcion'.$num_fila],$parametros['descripcion_corta'.$num_fila]);
							if($modificaAlojamientos_restaurantes == 'OK'){
								$Ntransacciones++;
							}else{
								$error = $modificaAlojamientos_restaurantes;
								
							}
						}else{

							$mAlojamientos_restaurantes = new clsAlojamientos_restaurantes($conexion, $filadesde, $usuario, $parametros['buscar_alojamiento']);
							$borraAlojamientos_restaurantes = $mAlojamientos_restaurantes->Borrar($parametros['id_alojamiento'.$num_fila],$parametros['nombre'.$num_fila]);
							if($borraAlojamientos_restaurantes == 'OK'){
								$Ntransacciones++;
							}else{
								$error = $borraAlojamientos_restaurantes;
				...
				//AÑADIR REGISTROS
				$num_filas_nuevas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'ALOJAMIENTOS_RESTAURANTES' AND USUARIO = '".$usuario."'");
						...
						if($filesize < $max){
							if($filesize > 0){
								if($type == 'image/jpeg' || $type == 'image/JPEG' || $type == 'image/jpg' || $type == 'image/JPG' 
									...
									$uploadfile = "imagenes/alojamientos/restaurantes/".$parametros['Nuevoid_alojamiento'.$num_fila]."_".$parametros['Nuevonombre'.$num_fila].".jpg";
									if (move_uploaded_file($_FILES['upfile'.$num_fila]['tmp_name'], $uploadfile)) {
										...
										$iAlojamientos_restaurantes = new clsAlojamientos_restaurantes($conexion, $filadesde, $usuario, $parametros['buscar_alojamiento']);
										$insertaAlojamientos_restaurantes = $iAlojamientos_restaurantes->Insertar($parametros['Nuevoid_alojamiento'.$num_fila], $parametros['Nuevonombre'.$num_fila], $parametros['Nuevodescripcion'.$num_fila], $parametros['Nuevodescripcion_corta'.$num_fila]);
										if($insertaAlojamientos_restaurantes == 'OK'){
											$Ntransacciones++;
										}else{
											$error = "<div><font color='#993300' size='3' ><b> Se ha producido el siguiente error: ".$insertaAlojamientos_restaurantes;
			...
			$oAlojamientos_restaurantes = new clsAlojamientos_restaurantes($conexion, $filadesde, $usuario, $parametros['buscar_alojamiento']);
			$sAlojamientos_restaurantes = $oAlojamientos_restaurantes->Cargar();
			$sAlojamientos_restaurantesnuevos = $oAlojamientos_restaurantes->Cargar_lineas_nuevas();
			$sComboSelectAlojamientos_restaurantes = $oAlojamientos_restaurantes->Cargar_combo_selector();
			...
			$smarty->assign('formulario', '» RESTAURANTES');
			...
			$smarty->assign('combo', $sComboSelectAlojamientos_restaurantes);

			//Cargar lineas de la tabla para visualizar modificar o borrar
			$smarty->assign('alojamientos_restaurantes', $sAlojamientos_restaurantes);
			...
			$smarty->assign('alojamientos_restaurantes_nuevos', $sAlojamientos_restaurantesnuevos);			
			...
			$smarty->display('plantillas/Alojamientos_restaurantes.html');
