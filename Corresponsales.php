<?php
	...
	require 'clases/corresponsales.cls.php';
	...
	$parametrosg = $_GET;
		...
		$mensaje2 = '';
		$error = '';
		$insertaCorresponsales = '';

	if(count($_POST) != 0){
		//esto es por si venimos de la pantalla acuerdos de corresponsales, servicios, o acuerdos de transportes
		...
		if($parametros['ir_a'] != 'S' and $parametros['ir_a'] != 'I'){
			...
			//--------------------------------------------------------------------------
			//---SECCION DEL CODIGO PARA LOS DATOS GENERALES DE LOS CORRESPONSALES----------
			//--------------------------------------------------------------------------

			$filadesde = $parametros['filadesde'];
			//Si se ha pulsado algun boton del selector de registros situamos el combo segun el boton pulsado
			if($parametros['botonSelector'] != 0){
				$botonCorresponsales = new clsCorresponsales($conexion, $filadesde, $usuario, $parametros['buscar_id'], $parametros['buscar_nombre']);
				$botonselec = $botonCorresponsales->Botones_selector($filadesde, $parametros['botonSelector']);
			...
			if($parametros['actuar'] == 'S'){
				...
				$num_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'CORRESPONSALES' AND USUARIO = '".$usuario."'");
			...
			$oCorresponsales = new clsCorresponsales($conexion, $filadesde, $usuario, $parametros['buscar_id'], $parametros['buscar_nombre']);
			$sCorresponsales = $oCorresponsales->Cargar($recuperaid,$recuperanombre,$recuperavisible,$recuperacif,$recuperalocalidad,$recuperadireccion,$recuperacodigo_postal,$recuperaprovincia,
			...
			$sComboSelectCorresponsales = $oCorresponsales->Cargar_combo_selector();
	...
	//-----------------------------------------------------------------------------------
			...
			//---------------------------------------------
			//----VISUALIZAR PARTE DE LOS CORRESPONSALES-------
			//---------------------------------------------
			...
			$smarty->assign('formulario', '» CORRESPONSALES');
			...
			$smarty->assign('combo', $sComboSelectCorresponsales);

			//Cargar lineas de la tabla para visualizar modificar o borrar
			$smarty->assign('corresponsales', $sCorresponsales);
			...
			$smarty->display('plantillas/Corresponsales.html');


		}elseif($parametros['ir_a'] == 'I'){

			header("Location: Corresponsales_imagenes.php?id=".$parametros['id0']);
		...
		//-----------------------------------------------------------
		//INICIALIZAMOS PARAMETROS Y PLANTILLA DE LOS CORRESPONSALES
