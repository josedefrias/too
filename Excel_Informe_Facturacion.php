...
//$conexion = conexion_hit();
					...
					$resultado =$conexion->query("select DATE_FORMAT(r.FECHA_SALIDA, '%d-%m-%Y') AS fecha_salida,
										...
										where 
												...
												order by r.FECHA_SALIDA,m.NOMBRE,r.localizador");
	...
	if($resultado->num_rows > 0 ){
		...
		if (PHP_SAPI == 'cli'){
			...
			$titulosColumnas = array('FECHA SALIDA', 'LOCALIZADOR', 'AGENCIA', 'OFICINA', 'DIRECCION', 'IMPORTE', 'MONEDA','ESTADO','FACTURA','FECHA FACTURA');
			...
			//Se agregan los datos de los alumnos
			 ...
			 while ($fila = $resultado->fetch_array()) {
				 $objPHPExcel->setActiveSheetIndex(0)
					 ...
					 ->setCellValue('J'.$i, $fila['fecha_factura']);
				 $i++;
			 }
