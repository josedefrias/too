...
//$conexion = conexion_hit();
	...
	if($resultado->num_rows > 0 ){
		...
		if (PHP_SAPI == 'cli'){
			...
			$tituloReporte = "Informe de Ventas Clientes: ".$buscar_salida_desde." hasta ".$buscar_salida_hasta;
			$titulosColumnas = array('FECHA SALIDA', 'LOCALIZADOR', 'AGENCIA', 'OFICINA', 'LOCALIDAD', 'PAX', 'VIAJE','PVP BRUTO','COMISIONABLE','NO COMISIONABLE','% COMISION',
				'IMPORTE COMISION','% IMPUESTO','IMPUESTO','FACTURA','PARA OVER', 'DIVISA','ESTADO');
			...
			//Se agregan los datos de los alumnos
			 ...
			 while ($fila = $resultado->fetch_array()) {
				 $objPHPExcel->setActiveSheetIndex(0)
					 ...
					 ->setCellValue('F'.$i, $fila['viaje'])
					 ->setCellValue('G'.$i, $fila['pax'])
					 ->setCellValue('H'.$i, $fila['pvp_bruto'])
					 ->setCellValue('I'.$i, $fila['comisionable'])
					 ->setCellValue('J'.$i, $fila['no_comisionable'])
					 ->setCellValue('K'.$i, $fila['comision'])
					 ->setCellValue('L'.$i, $fila['importe_comision'])
					 ->setCellValue('M'.$i, $fila['impuesto'])
					 ->setCellValue('N'.$i, $fila['importe_impuesto'])
					 ->setCellValue('O'.$i, $fila['importe_facturado'])
					 ->setCellValue('P'.$i, $fila['importe_sintasas'])
					 ->setCellValue('Q'.$i, $fila['moneda'])
					 ->setCellValue('R'.$i, $fila['estado']);
				 $i++;
			 }
			...
			$objPHPExcel->getActiveSheet()->setTitle('Ventas_Agencias');
			...
			header('Content-Disposition: attachment;filename="Ventas_Agencias.xlsx"');
