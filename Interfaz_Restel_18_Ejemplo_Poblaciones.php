<?php
	...
	$resultado =$conexion->query("select codigo from hit_interfaces_provincias where pais n in ('ES')");
		...
		try {
			...
			for($i=0;$i<$no_hotels;$i++)
			{
				...
				print('CODIGO DE ESTADO: '.$poblacion->deppob.'-'.
				        ...
				        ' NOMBRE: '.$poblacion->poblidinom1.'<br>'); // prints te hotelname etc.
				//print($i);


				/*$query = "INSERT INTO hit_interfaces_poblaciones (INTERFAZ, CODIGO, PROVINCIA, NOMBRE) VALUES (";
				$query .= "'RESTEL',";
				$query .= "'".$poblacion->codpob."',";
				$query .= "'".$poblacion->deppob."',";
				$query .= "'".mysql_real_escape_string($poblacion->poblidinom1)."')";
	...
	print("<br>Numero de localidades:".$numero_localidades);
?>