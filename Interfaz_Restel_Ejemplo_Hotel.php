<?php
	...
	$xml2 .= "<peticion>\n";
	$xml2 .= "<tipo>15</tipo>\n";
	$xml2 .= "<nombre>Petición de Informacion de Hotel</nombre>\n";
	...
	$no_hotels = count($xml->parametros->hotel->hotel);
	//ECHO($respuesta.'<br><br><br>');
	//ECHO($no_hotels.'<br><br><br>');
	for($i=0;$i<$no_hotels;$i++)
	{
		$hotel = $xml->parametros->hotel->hotel[$i];
		print('CODIGO:'.$hotel->codigo.'<br>'.
		        ...
		        'CATEGORIA 2:'.$hotel->categoria2.'<br>'.
		        'CODIGO COBOL:'.$hotel->codigo_cobol.'<br>---------------------------------------------------------------<br>'); // prints te hotelname etc.
		//print($i);
