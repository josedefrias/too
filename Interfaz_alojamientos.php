<?php
	...
	require 'clases/interfaz_alojamientos.cls.php';
	...
	if(count($_POST) != 0){
		...
		if($parametros['ir_a'] != 'S'){
			...
			$oInterfaz_alojamientos = new clsInterfaz_alojamientos($conexion, $filadesde, $usuario, $parametros['buscar_interfaz'], $parametros['buscar_provincia'], $parametros['buscar_poblacion'], $parametros['buscar_categoria'], $parametros['buscar_hotel']);
			$sInterfaz_alojamientos = $oInterfaz_alojamientos->Cargar();
			//lineas visualizadas
			$vueltas = count($sInterfaz_alojamientos);
			//echo($vueltas);
			...
			if($parametros['actuar'] == 'S'){
				...
				for ($num_fila = 0; $num_fila <= $vueltas-1; $num_fila++) {
					if($parametros['selec'.$num_fila] == 'S' ){
						...
						$Interfaz_alojamientos = new clsInterfaz_alojamientos($conexion, $filadesde, $usuario, $parametros['buscar_interfaz'], $parametros['buscar_provincia'], $parametros['buscar_poblacion'], $parametros['buscar_categoria'], $parametros['buscar_hotel']);

						$insertaInterfaz_alojamientos = $Interfaz_alojamientos->Insertar($parametros['codigo'.$num_fila],$parametros['ciudad_provincia'.$num_fila],$parametros['alojamiento_hits'.$num_fila],$parametros['divisa'.$num_fila]);

						if($insertaInterfaz_alojamientos == 'OK'){
							$Ntransacciones++;
						}else{
							$error = $insertaInterfaz_alojamientos;
						}
					}
				}
			...
			$smarty->assign('interfaz_alojamientos', $sInterfaz_alojamientos);
			$smarty->assign('cantidad_hoteles', $vueltas);

			//$smarty->assign('combo', $sComboSelectInterfaz_alojamientos);
			...
			$smarty->display('plantillas/Interfaz_alojamientos.html');
		...
		$oInterfaz_alojamientos = new clsInterfaz_alojamientos($conexion, $filadesde, $usuario, $usuario, $parametros['buscar_interfaz'], $parametros['buscar_provincia'], $parametros['buscar_poblacion'], $parametros['buscar_categoria'], $parametros['buscar_hotel']);
		$sInterfaz_alojamientos = $oInterfaz_alojamientos->Cargar();
		//$sInterfaz_alojamientosnuevos = $oInterfaz_alojamientos->Cargar_lineas_nuevas();
		//$sComboSelectInterfaz_alojamientos = $oInterfaz_alojamientos->Cargar_combo_selector();
		...
		/*$smarty->assign('interfaz_alojamientos_nuevos', $sInterfaz_alojamientosnuevos);
