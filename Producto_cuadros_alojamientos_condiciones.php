<?php
	...
	require 'clases/producto_cuadros_alojamientos_condiciones.cls.php';
...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	$ClaseCondiciones = new clsCuadros_alojamientos_condiciones($conexion, $filadesde, $usuario, $parametros['buscar_codigo'], $parametros['buscar_cuadro']);
...
//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------------
			...
			if($parametros['botonSelector_2nivel'] != 0){
				$botonselec_condiciones = $ClaseCondiciones->Botones_selector_alojamientos_condiciones($filadesde_condiciones, $parametros['botonSelector_2nivel']);
				$filadesde_condiciones = $botonselec_condiciones;
			}

			//Llamada a la clase especifica de la pantalla
			$sCondiciones = $ClaseCondiciones->Cargar_alojamientos_condiciones($sCuadros[0]['clave'], $filadesde_condiciones, $parametros['buscar_paquete'], $parametros['buscar_tipo']);	
			//lineas visualizadas
			...
			if($parametros['actuar_2nivel'] == 'S'){
				...
				//MODIFICAR Y BORRAR REGISTROS
				/*$num_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'PRODUCTO_CUADROS_ALOJAMIENTOS_CONDICIONES' AND USUARIO = '".$usuario."'");
				$Nfilas	 = $num_filas->fetch_assoc();*/
				...
				for ($num_fila = 0; $num_fila <= $vueltas-1; $num_fila++) {
					if($parametros['selec_2nivel'.$num_fila] == 'S' || $parametros['borra_2nivel'.$num_fila] == 'S'){
						if($parametros['selec_2nivel'.$num_fila] == 'S'){
							...
							$modificaCondiciones = $ClaseCondiciones->Modificar_alojamientos_condiciones($sCuadros[0]['clave'], $parametros['paquete'.$num_fila], $parametros['tipo'.$num_fila], $parametros['uso'.$num_fila], $parametros['salida_desde'.$num_fila], $parametros['salida_hasta'.$num_fila], $parametros['reserva_desde'.$num_fila],$parametros['reserva_hasta'.$num_fila],$parametros['margen_1'.$num_fila],$parametros['margen_2'.$num_fila],$parametros['maximo'.$num_fila],$parametros['forma_calculo'.$num_fila],$parametros['valor_1'.$num_fila],$parametros['valor_2'.$num_fila], $parametros['acumulable'.$num_fila], $parametros['prioritario'.$num_fila], $parametros['aplicacion'.$num_fila], $parametros['regimen'.$num_fila], $parametros['paquete_old'.$num_fila], $parametros['tipo_old'.$num_fila], $parametros['uso_old'.$num_fila], $parametros['salida_desde_old'.$num_fila], $parametros['salida_hasta_old'.$num_fila], $parametros['reserva_desde_old'.$num_fila],$parametros['reserva_hasta_old'.$num_fila],$parametros['margen_1_old'.$num_fila],$parametros['margen_2_old'.$num_fila]);
							...
							$borraCondiciones = $ClaseCondiciones->Borrar_alojamientos_condiciones($sCuadros[0]['clave'], $parametros['paquete'.$num_fila], $parametros['tipo'.$num_fila], $parametros['uso'.$num_fila], $parametros['salida_desde'.$num_fila], $parametros['salida_hasta'.$num_fila], $parametros['reserva_desde'.$num_fila],$parametros['reserva_hasta'.$num_fila],$parametros['margen_1'.$num_fila],$parametros['margen_2'.$num_fila]);
				...
				//AÑADIR REGISTROS
				$num_filas_nuevas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'PRODUCTO_CUADROS_ALOJAMIENTOS_CONDICIONES' AND USUARIO = '".$usuario."'");
				...
				for ($num_fila = 0; $num_fila <= $Nfilas_nuevas['LINEAS_NUEVAS']-1; $num_fila++) {
					...
					if($parametros['Nuevoselec_2nivel'.$num_fila] == 'S'){
						...
						$insertaCondiciones = $ClaseCondiciones->Insertar_alojamientos_condiciones($sCuadros[0]['clave'], $sCuadros[0]['folleto'], $sCuadros[0]['cuadro'], $parametros['Nuevopaquete'.$num_fila], $parametros['Nuevotipo'.$num_fila], $parametros['Nuevouso'.$num_fila], $parametros['Nuevosalida_desde'.$num_fila],$parametros['Nuevosalida_hasta'.$num_fila],$parametros['Nuevoreserva_desde'.$num_fila],$parametros['Nuevoreserva_hasta'.$num_fila],$parametros['Nuevomargen_1'.$num_fila],$parametros['Nuevomargen_2'.$num_fila],$parametros['Nuevomaximo'.$num_fila],$parametros['Nuevoforma_calculo'.$num_fila],$parametros['Nuevovalor_1'.$num_fila],$parametros['Nuevovalor_2'.$num_fila],$parametros['Nuevoacumulable'.$num_fila],$parametros['Nuevoprioritario'.$num_fila],$parametros['Nuevoaplicacion'.$num_fila],$parametros['Nuevoregimen'.$num_fila]);
				...
				$respuesta_error = '';
				$calcular = "CALL `PRODUCTO_CALCULA_CONDICIONES_ALOJAMIENTOS`('".$parametros['folleto0']."', '".$parametros['cuadro0']."')";
				$resultadocalcular =$conexion->query($calcular);
					//echo($expandir);
				if ($resultadocalcular == FALSE){
					$respuesta_error = 'No se han podido calcular las condiciones. '.$conexion->error;
				}else{
					$respuesta = 'Condiciones calculadas';
				...
				$respuesta_error = '';
				$calcular = "CALL `PRODUCTO_RECALCULA_CONDICIONES_ALOJAMIENTOS`('".$parametros['folleto0']."', '".$parametros['cuadro0']."')";
				$resultadocalcular =$conexion->query($calcular);
					//echo($expandir);
				if ($resultadocalcular == FALSE){
					$respuesta_error = 'No se han podido calcular las condiciones. '.$conexion->error;
			...
			//Llamada a la clase especifica de la pantalla
			$sCondiciones = $ClaseCondiciones->Cargar_alojamientos_condiciones($sCuadros[0]['clave'], $filadesde_condiciones, $parametros['buscar_paquete'], $parametros['buscar_tipo']);	
			$sComboSelectCondiciones = $ClaseCondiciones->Cargar_combo_selector_alojamientos_condiciones($sCuadros[0]['clave'], $filadesde_condiciones, $parametros['buscar_paquete'], $parametros['buscar_tipo']);
			$sCondiciones_nuevos = $ClaseCondiciones->Cargar_lineas_nuevas_alojamientos_condiciones();	
			...
			$comboRegimen = $combos->Cargar_combo_Regimen_Producto();
...
//----------------------------------------------------------------------------------------------------------------------------------------------
			...
			$smarty->assign('formulario', '» ALOJAMIENTOS - CONDICIONES');
...
//----------------------------------------------------------------------------------------------------------------------------------------------
			...
			$smarty->display('plantillas/Producto_cuadros_alojamientos_condiciones.html');
...
//----------------------------------------------------------------------------------------------------------------------------------------------
			...
			//Llamada a la clase especifica de la pantalla
			$sCuadros = $ClaseCuadros->Cargar($recuperacodigo,$recuperacuadro,$recuperaproducto,$recuperatipo,$recuperanombre_cuadro,$recuperadestino_cuadro, $recuperadestino_cuadro2,$recuperasituacion,$recuperaduracion,$recuperaduracion,$recuperadias_operacion,$recuperafrecuencia,$recuperaprimera_salida,$recuperaultima_salida,$recuperadivisa,$recuperaredondeo,$recuperacodigo_administrativo,$recuperamargen_transportes,$recuperaocupacion_transportes,$recuperamargen_alojamientos,$recuperamargen_alojamientos_interfaz_1,$recuperamargen_servicios, $recuperaventa,$recuperaminorista_margen_transportes,$recuperaminorista_ocupacion_transportes,$recuperaminorista_margen_alojamientos,$recuperaminorista_margen_alojamientos_interfaz_1,$recuperaminorista_margen_servicios);
			$sComboSelectCuadros = $ClaseCuadros->Cargar_combo_selector();
