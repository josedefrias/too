...
class clsGrupos_Minoristas{
	...
	function Cargar($grupo){
		...
		if($buscar_codigo != null){
			...
			$CADENA_BUSCAR = " WHERE GRUPO_GESTION = '".$grupo."'";
		}else{
			$CADENA_BUSCAR = "";
		}

		$resultado =$conexion->query("SELECT id, nombre, telefono, mail
									  FROM hit_minoristas ".$CADENA_BUSCAR." ORDER BY NOMBRE");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'GRUPOS_MINORISTAS' AND USUARIO = '".$Usuario."'");
	...
	function Cargar_combo_selector($grupo){
	...
	function Botones_selector($filadesde, $boton, $grupo){
	...
	function clsGrupos_Minoristas($conexion, $filadesde, $usuario, $buscar_codigo, $buscar_nombre){
