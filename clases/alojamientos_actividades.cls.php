...
class clsAlojamientos_actividades{
	...
	function Cargar(){
		...
		$resultado =$conexion->query("SELECT ah.ID id, ah.ID_ALOJAMIENTO id_alojamiento, ah.NOMBRE nombre, ah.DESCRIPCION descripcion, ah.DESCRIPCION_CORTA descripcion_corta
									  FROM hit_alojamientos_actividades ah, hit_alojamientos a
										where
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'ALOJAMIENTOS_ACTIVIDADES' AND USUARIO = '".$Usuario."'");
		$Nfilas	 = $numero_filas->fetch_assoc();											                        //------

		$alojamientos_actividades = array();
		for ($num_fila = $Filadesde-1; $num_fila <= $Filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
			$resultado->data_seek($num_fila);
			$fila = $resultado->fetch_assoc();
			array_push($alojamientos_actividades,$fila);
		}
		...
		return $alojamientos_actividades;											
	}
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'ALOJAMIENTOS_ACTIVIDADES' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query("SELECT * FROM hit_alojamientos_actividades ah, hit_alojamientos a
	...
	function Modificar($id,$nombre, $descripcion, $descripcion_corta){
		...
		$conexion = $this ->Conexion;
		$query = "UPDATE hit_alojamientos_actividades SET ";
		$query .= " NOMBRE = '".$nombre."'";
	...
	function Borrar($id_alojamiento,$nombre){
		...
		$query = "DELETE FROM hit_alojamientos_actividades WHERE ID_ALOJAMIENTO = '".$id_alojamiento."' and NOMBRE = '".$nombre."'";
		...
		if ($resultadob == FALSE){
			...
			$fichero_imagen = "imagenes/alojamientos/actividades/".$id_alojamiento."_".$nombre.".jpg";
			//echo($fichero_imagen);
	...
	function Insertar($id_alojamiento, $nombre, $descripcion, $descripcion_corta){
		...
		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_alojamientos_actividades (ID_ALOJAMIENTO, NOMBRE, DESCRIPCION, DESCRIPCION_CORTA) VALUES (";
	...
	function clsAlojamientos_actividades($conexion, $filadesde, $usuario, $buscar_alojamiento){
