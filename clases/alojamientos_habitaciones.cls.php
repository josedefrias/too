...
class clsAlojamientos_habitaciones{
	...
	function Cargar(){
		...
		$resultado =$conexion->query("SELECT ah.ID id, ah.ID_ALOJAMIENTO id_alojamiento, ah.CODIGO codigo, ah.NOMBRE nombre, ah.DESCRIPCION descripcion, ah.DESCRIPCION_CORTA descripcion_corta
									  FROM hit_alojamientos_habitaciones ah, hit_alojamientos a
										where
											ah.ID_ALOJAMIENTO = a.ID ".$CADENA_BUSCAR." ORDER BY ah.CODIGO");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'ALOJAMIENTOS_HABITACIONES' AND USUARIO = '".$Usuario."'");
		$Nfilas	 = $numero_filas->fetch_assoc();											                        //------

		$alojamientos_habitaciones = array();
		for ($num_fila = $Filadesde-1; $num_fila <= $Filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
			$resultado->data_seek($num_fila);
			$fila = $resultado->fetch_assoc();
			array_push($alojamientos_habitaciones,$fila);
		}
		...
		return $alojamientos_habitaciones;											
	}
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'ALOJAMIENTOS_HABITACIONES' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query("SELECT * FROM hit_alojamientos_habitaciones ah, hit_alojamientos a
	...
	function Modificar($id,$codigo, $nombre, $descripcion, $descripcion_corta){

		$conexion = $this ->Conexion;
		$query = "UPDATE hit_alojamientos_habitaciones SET ";
		$query .= " CODIGO = '".$codigo."'";
		$query .= ", NOMBRE = '".$nombre."'";
	...
	function Borrar($id_alojamiento, $codigo){

		$conexion = $this ->Conexion;


		$query = "DELETE FROM hit_alojamientos_habitaciones WHERE ID_ALOJAMIENTO = '".$id_alojamiento."' and CODIGO = '".$codigo."'";
		...
		if ($resultadob == FALSE){
			...
			$fichero_imagen = "imagenes/alojamientos/habitaciones/".$id_alojamiento."_".$codigo.".jpg";
			//echo($fichero_imagen);
	...
	function Insertar($id_alojamiento, $codigo, $nombre, $descripcion, $descripcion_corta){

		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_alojamientos_habitaciones (ID_ALOJAMIENTO, CODIGO, NOMBRE, DESCRIPCION, DESCRIPCION_CORTA) VALUES (";
	...
	function clsAlojamientos_habitaciones($conexion, $filadesde, $usuario, $buscar_alojamiento){
