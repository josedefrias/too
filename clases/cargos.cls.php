...
class clsCargos{
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'CARGOS' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_cargos'.$CADENA_BUSCAR);
	...
	function Borrar($codigo){
		...
		$query = "DELETE FROM hit_cargos WHERE CODIGO = '".$codigo."'";
	...
	function clsCargos($conexion, $filadesde, $usuario, $buscar_codigo, $buscar_nombre){
