...
class clsCiudades{
	...
	function Cargar(){
		...
		if($buscar_codigo != null){
			...
			$CADENA_BUSCAR = " WHERE PROVINCIA = '".$buscar_provincia."'";
		}elseif($buscar_zona != null){
			$CADENA_BUSCAR = " WHERE ZONA = '".$buscar_zona."'";
		}else{
			$CADENA_BUSCAR = "";
		}

		$resultado =$conexion->query("SELECT codigo,nombre,pais,provincia, zona, visible_hits, visible_web
									  FROM hit_ciudades ".$CADENA_BUSCAR." ORDER BY NOMBRE");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'CIUDADES' AND USUARIO = '".$Usuario."'");
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'CIUDADES' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_ciudades'.$CADENA_BUSCAR);
	...
	function Modificar($codigo, $nombre, $pais, $provincia, $zona, $visible_hits, $visible_web){
	...
	function Borrar($codigo){
		...
		$query = "DELETE FROM hit_ciudades WHERE CODIGO = '".$codigo."'";
	...
	function Insertar($codigo, $nombre, $pais, $provincia, $zona, $visible_hits, $visible_web){

		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_ciudades (CODIGO, NOMBRE, PAIS, PROVINCIA, ZONA, VISIBLE_HITS, VISIBLE_WEB) VALUES (";
		$query .= "'".$codigo."',";
		$query .= "'".$nombre."',";
		$query .= "'".$pais."',";
		$query .= "'".$provincia."',";
	...
	function clsCiudades($conexion, $filadesde, $usuario, $buscar_codigo, $buscar_nombre, $buscar_provincia, $buscar_zona){
