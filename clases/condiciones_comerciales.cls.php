...
class clsCondiciones_comerciales{
	...
	function Cargar(){
		...
		$resultado =$conexion->query("SELECT codigo, nombre, visible, tipo, grupo, orden_calculo, descripcion
									  FROM hit_condiciones_comerciales ORDER BY TIPO, GRUPO, ORDEN_CALCULO");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'CONDICIONES_COMERCIALES' AND USUARIO = '".$Usuario."'");
		...
		for ($num_fila = $Filadesde-1; $num_fila <= $Filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
			...
			array_push($condiciones_comerciales,$fila);
		}
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'CONDICIONES_COMERCIALES' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_condiciones_comerciales');
	...
	function Modificar($codigo, $nombre, $visible, $tipo, $grupo, $orden_calculo, $descripcion){

		$conexion = $this ->Conexion;
		$query = "UPDATE hit_condiciones_comerciales SET ";
		$query .= " NOMBRE = '".$nombre."'";
		$query .= ", VISIBLE = '".$visible."'";
		$query .= ", TIPO = '".$tipo."'";
		$query .= ", GRUPO = '".$grupo."'";
		$query .= ", ORDEN_CALCULO = '".$orden_calculo."'";
	...
	function Borrar($codigo){
		...
		$query = "DELETE FROM hit_condiciones_comerciales WHERE CODIGO = '".$codigo."'";
	...
	function Insertar($codigo, $nombre, $visible, $tipo, $grupo, $orden_calculo, $descripcion){

		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_condiciones_comerciales (CODIGO, NOMBRE, VISIBLE, TIPO, GRUPO, ORDEN_CALCULO,DESCRIPCION) VALUES (";
		$query .= "'".$codigo."',";
		$query .= "'".$nombre."',";
		$query .= "'".$visible."',";
		$query .= "'".$tipo."',";
	...
	function clsCondiciones_comerciales($conexion, $filadesde, $usuario){
