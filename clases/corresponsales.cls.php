...
//------------------------------------------------------------------
	function Cargar($recuperaid,$recuperanombre,$recuperavisible,$recuperacif,$recuperalocalidad,$recuperadireccion,$recuperacodigo_postal,$recuperaprovincia,
		...
		*/
		...
		FROM hit_corresponsales ".$CADENA_BUSCAR." ORDER BY nombre");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'CORRESPONSALES' AND USUARIO = '".$Usuario."'");
		$Nfilas	 = $numero_filas->fetch_assoc();											                        //------

		$corresponsales = array();
		if($recuperanombre != null){
			$corresponsales[0] = array ("id" => $recuperaid,"nombre" => $recuperanombre,"visible" => $recuperavisible,"cif" => $recuperacif,"localidad" => $recuperalocalidad,
			...
			for ($num_fila = $Filadesde-1; $num_fila <= $Filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
				...
				array_push($corresponsales,$fila);
			}
		}
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_corresponsales'.$CADENA_BUSCAR);
	...
	function Modificar($id,$nombre,$visible,$cif,$localidad,$direccion,$codigo_postal,$provincia,$pais,$telefono,$fax,$email,$caracteristicas,
		...
		$conexion = $this ->Conexion;
		$query = "UPDATE hit_corresponsales SET ";
		$query .= " NOMBRE = '".$nombre."'";
	...
	function Borrar($id){
		...
		$query = "DELETE FROM hit_corresponsales WHERE ID = '".$id."'";
	...
	function Insertar($nombre,$visible,$cif,$localidad,$direccion,$codigo_postal,$provincia,$pais,$telefono,$fax,$email,$caracteristicas,
		...
		$query = "INSERT INTO hit_corresponsales (NOMBRE,VISIBLE,CIF, LOCALIDAD,DIRECCION,CODIGO_POSTAL,PROVINCIA,PAIS,TELEFONO,FAX,EMAIL, CARACTERISTICAS,IDIOMA,RESPONSABLE,CARGO_RESPONSABLE,URL,SWIFT,CC_IBAN,NOMBRE_BANCO,DIRECCION_BANCO) VALUES (";
		$query .= "'".$nombre."',"; 
		$query .= "'".$visible."',";
		$query .= "'".$cif."',";
		$query .= "'".$localidad."',";
	...
	function clsCorresponsales($conexion, $filadesde, $usuario, $buscar_id, $buscar_nombre){
