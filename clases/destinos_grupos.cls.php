...
class clsDestinos_grupos{
	...
	function Cargar(){
		...
		$resultado =$conexion->query("SELECT codigo ,nombre, orden_visualizacion
									  FROM hit_destinos_grupos ".$CADENA_BUSCAR." ORDER BY NOMBRE");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'DESTINOS_GRUPOS' AND USUARIO = '".$Usuario."'");
		$Nfilas	 = $numero_filas->fetch_assoc();											                        //------

		$destinos_grupos = array();
		for ($num_fila = $Filadesde-1; $num_fila <= $Filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
			...
			array_push($destinos_grupos,$fila);
		}
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'DESTINOS_GRUPOS' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_destinos_grupos'.$CADENA_BUSCAR);
	...
	function Modificar($codigo, $nombre, $orden_visualizacion){

		$conexion = $this ->Conexion;
		$query = "UPDATE hit_destinos_grupos SET ";
		$query .= " NOMBRE = '".$nombre."'";
		$query .= ", ORDEN_VISUALIZACION = '".$orden_visualizacion."'";
	...
	function Borrar($codigo){
		...
		$query = "DELETE FROM hit_destinos_grupos WHERE CODIGO = '".$codigo."'";
	...
	function Insertar($codigo, $nombre, $orden_visualizacion){

		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_destinos_grupos (CODIGO, NOMBRE, ORDEN_VISUALIZACION) VALUES (";
		$query .= "'".$codigo."',";
		$query .= "'".$nombre."',";
		$query .= "'".$orden_visualizacion."')";
	...
	function clsDestinos_grupos($conexion, $filadesde, $usuario, $buscar_codigo, $buscar_nombre){
