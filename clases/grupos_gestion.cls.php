...
//------------------------------------------------------------------
	function Cargar($recuperanombre,$recuperacif,$recuperavisible,$recuperacic,$recuperadomicilio,$recuperaciudad,$recuperacodigo_postal,	$recuperaprovincia,$recuperapais,$recuperatelefono,$recuperafax,$recuperamail,$recuperaresponsable,$recuperacargo_responsable, $recuperagrupo_comision, $recuperaenviar_ofertas){
		...
		$resultado =$conexion->query("SELECT id,nombre,cif,visible,cic,domicilio,ciudad,codigo_postal,provincia,pais,telefono,fax,mail,responsable,cargo_responsable, grupo_comision, enviar_ofertas FROM hit_grupos_gestion ".$CADENA_BUSCAR." ORDER BY NOMBRE");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'GRUPOS_GESTION' AND USUARIO = '".$Usuario."'");
		$Nfilas	 = $numero_filas->fetch_assoc();											                        //------

		$grupos_gestion = array();
		if($recuperanombre != null){
			$grupos_gestion[0] = array ( "id" => null, "nombre" => $recuperanombre, "cif" => $recuperacif, "visible" => $recuperavisible, "cic" => $recuperacic, "domicilio" => $recuperadomicilio, "ciudad" => $recuperaciudad, "codigo_postal" => $recuperacodigo_postal, "provincia" => $recuperaprovincia, "pais" => $recuperapais, "telefono" => $recuperatelefono, "fax" => $recuperafax, "mail" => $recuperamail, "responsable" => $recuperaresponsable, "cargo_responsable" => $recuperacargo_responsable, "grupo_comision" => $recuperagrupo_comision);
		}else{
			for ($num_fila = $Filadesde-1; $num_fila <= $Filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
				$resultado->data_seek($num_fila);
				$fila = $resultado->fetch_assoc();
				array_push($grupos_gestion,$fila);
			}
		}
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'GRUPOS_GESTION' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_grupos_gestion'.$CADENA_BUSCAR);
	...
	function Modificar($codigo,$nombre,$cif,$visible,$cic,$domicilio,$ciudad,$codigo_postal,$provincia,$pais,$telefono,$fax,$mail,$responsable,$cargo_responsable,$grupo_comision,$enviar_ofertas){

		$conexion = $this ->Conexion;
		$query = "UPDATE hit_grupos_gestion SET ";
		$query .= " NOMBRE = '".$nombre."'";
		$query .= ", CIF = '".$cif."'";
		...
		$query .= ", ENVIAR_OFERTAS = '".$enviar_ofertas."'";
	...
	function Borrar($codigo){
		...
		$query = "DELETE FROM hit_grupos_gestion WHERE ID = '".$codigo."'";
	...
	function Insertar($nombre,$cif,$visible,$cic,$domicilio,$ciudad,$codigo_postal,$provincia,$pais,$telefono,$fax,$mail,$responsable,$cargo_responsable,$grupo_comision,$enviar_ofertas){

		$conexion = $this ->Conexion;

		$query = "INSERT INTO hit_grupos_gestion (NOMBRE, CIF, VISIBLE, CIC, DOMICILIO, CIUDAD, CODIGO_POSTAL, PROVINCIA, PAIS, TELEFONO, FAX, MAIL, RESPONSABLE, CARGO_RESPONSABLE, GRUPO_COMISION, ENVIAR_OFERTAS) VALUES (";
		$query .= "'".$nombre."',";
		$query .= "'".$cif."',";
		$query .= "'".$visible."',";
		$query .= "'".$cic."',";
...
//--------------------------------------------------------------------
	...
	function Cargar_comisiones($id, $filadesde, $buscar_producto){
		...
		$resultado =$conexion->query("SELECT DATE_FORMAT(fecha_desde, '%d-%m-%Y') AS fecha_desde, DATE_FORMAT(fecha_hasta, '%d-%m-%Y') AS fecha_hasta, producto, folleto, cuadro, paquete, comision_paquetes, comision_alojamientos, comision_transportes,	
									  comision_servicios FROM hit_grupos_comisiones ".$CADENA_BUSCAR." ORDER BY fecha_desde, fecha_hasta, producto, folleto, cuadro, paquete");
		...
		//Guardamos el resultado en una matriz con un numero fijo de registros
		//que controlaremos por una tabla de configuracion de pantallas de usuarios. ESPECIFICO: Solo el nombre del formulario en la query
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'GRUPOS_COMISIONES' AND USUARIO = '".$Usuario."'");
		$Nfilas	 = $numero_filas->fetch_assoc();											                        //------

		$grupos_gestion_comisiones = array();
		for ($num_fila = $filadesde-1; $num_fila <= $filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
			$resultado->data_seek($num_fila);
			$fila = $resultado->fetch_assoc();
			...
			array_push($grupos_gestion_comisiones,$fila);
		}
		...
		return $grupos_gestion_comisiones;											
	}
	...
	function Cargar_lineas_nuevas_comisiones(){
		...
		//Cargamos el array para el combo con las lineas nuevas. (n lineas se obtendran de la tabla de usuarios_formularios).
		//																						  ESPECIFICO: Solo el nombre del formulario en la query
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'GRUPOS_COMISIONES' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
			echo('error al leer lineas nuevas');
			$Nfilasnuevas = 2;
		}
	...
	function Cargar_combo_selector_comisiones($id, $buscar_producto){
		...
		if($buscar_producto != null){
			...
			$CADENA_BUSCAR = " WHERE ID = '".$id."'";
												...
												//ESPCIFICO: nombre tabla
		$resultadoc =$conexion->query('SELECT * FROM hit_grupos_comisiones'.$CADENA_BUSCAR);
		$numero_filas = $resultadoc->num_rows;         //----------
	...
	function Modificar_comisiones($id, $fecha_desde, $fecha_hasta, $producto, $folleto, $cuadro, $paquete, $comision_paquetes, $comision_alojamientos, $comision_transportes, $comision_servicios){
		...
		$conexion = $this ->Conexion;
		$query = "UPDATE hit_grupos_comisiones SET ";
	...
	function Borrar_comisiones($id, $fecha_desde,$fecha_hasta,$producto,$folleto,$cuadro,$paquete){
		...
		$query = "DELETE FROM hit_grupos_comisiones WHERE ID = '".$id."'";
		$query .= " AND FECHA_DESDE = '".date("Y-m-d",strtotime($fecha_desde))."'";
		$query .= " AND FECHA_HASTA = '".date("Y-m-d",strtotime($fecha_hasta))."'";
		$query .= " AND PRODUCTO = '".$producto."'";
		$query .= " AND FOLLETO = '".$folleto."'";
	...
	function Insertar_comisiones($id, $fecha_desde, $fecha_hasta, $producto, $folleto, $cuadro, $paquete, $comision_paquetes, $comision_alojamientos, $comision_transportes, $comision_servicios){
		...
		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_grupos_comisiones (ID, FECHA_DESDE, FECHA_HASTA, PRODUCTO, FOLLETO, CUADRO, PAQUETE, COMISION_PAQUETES, COMISION_ALOJAMIENTOS, COMISION_TRANSPORTES, COMISION_SERVICIOS) VALUES (";
		$query .= "'".$id."',";
	...
	function clsGrupos_gestion($conexion, $filadesde, $usuario, $buscar_codigo, $buscar_nombre){
