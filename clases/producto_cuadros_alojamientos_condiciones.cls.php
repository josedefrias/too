...
class clsCuadros_alojamientos_condiciones{
...
//--------------------------------------------------------------------
	...
	function Cargar_lineas_nuevas_alojamientos_condiciones(){
		$Usuario = $this ->Usuario;
		$conexion = $this ->Conexion;		
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'PRODUCTO_CUADROS_ALOJAMIENTOS_CONDICIONES' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector_alojamientos_condiciones($clave, $filadesde, $buscar_paquete, $buscar_tipo){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_producto_cuadros_alojamientos_condiciones'.$CADENA_BUSCAR);
	...
	function Botones_selector_alojamientos_condiciones($filadesde, $boton){

		$Usuario = $this ->Usuario;
		$conexion = $this ->Conexion;		
												//ESPCIFICO: nombre tabla
		$resultadoc =$conexion->query('SELECT * FROM hit_producto_cuadros_alojamientos_condiciones');
		$numero_filas = $resultadoc->num_rows;         //----------
		if ($numero_filas == FALSE){
	...
	function Modificar_alojamientos_condiciones($clave_cuadro, $paquete, $tipo, $uso, $salida_desde, $salida_hasta, $reserva_desde, $reserva_hasta, $margen_1, $margen_2, $maximo, $forma_calculo, $valor_1,
		$valor_2, $acumulable, $prioritario, $aplicacion, $regimen, $paquete_old,
		$tipo_old, $uso_old, $salida_desde_old, $salida_hasta_old, $reserva_desde_old, $reserva_hasta_old, $margen_1_old, $margen_2_old){

		$conexion = $this ->Conexion;
		$query = "UPDATE hit_producto_cuadros_alojamientos_condiciones SET ";
	...
	function Borrar_alojamientos_condiciones($clave_cuadro, $paquete, $tipo, $uso, $salida_desde, $salida_hasta, $reserva_desde, $reserva_hasta, $margen_1, $margen_2){

		$conexion = $this ->Conexion;

		$query = "DELETE FROM hit_producto_cuadros_alojamientos_condiciones WHERE CLAVE_CUADRO = '".$clave_cuadro."'";
	...
	function Insertar_alojamientos_condiciones($clave_cuadro, $folleto, $cuadro, $paquete, $tipo, $uso, $salida_desde, $salida_hasta, $reserva_desde, $reserva_hasta, $margen_1, $margen_2, $maximo, $forma_calculo, $valor_1, $valor_2, $acumulable, $prioritario, $aplicacion, $regimen){

		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_producto_cuadros_alojamientos_condiciones (CLAVE_CUADRO, FOLLETO, CUADRO, PAQUETE, TIPO, USO, SALIDA_DESDE, SALIDA_HASTA, RESERVA_DESDE, RESERVA_HASTA, MARGEN_1, MARGEN_2, MAXIMO, FORMA_CALCULO, VALOR_1, VALOR_2, ACUMULABLE, PRIORITARIO, APLICACION, REGIMEN) VALUES (";
		$query .= "'".$clave_cuadro."',";
		$query .= "'".$folleto."',";
		$query .= "'".$cuadro."',";
		$query .= "'".$paquete."',";
	...
	function clsCuadros_alojamientos_condiciones($conexion, $filadesde, $usuario, $buscar_codigo, $buscar_cuadro){
