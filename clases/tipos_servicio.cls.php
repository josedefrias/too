...
class clsTipos_servicio{
	...
	function Cargar(){
		...
		$resultado =$conexion->query("SELECT codigo,nombre,venta_web
									  FROM hit_tipos_servicio ".$CADENA_BUSCAR." ORDER BY NOMBRE");
		...
		$numero_filas =$conexion->query("SELECT LINEAS_MODIFICACION FROM hit_usuarios_formularios WHERE FORMULARIO = 'TIPOS_SERVICIO' AND USUARIO = '".$Usuario."'");
		...
		for ($num_fila = $Filadesde-1; $num_fila <= $Filadesde + $Nfilas['LINEAS_MODIFICACION']-2; $num_fila++) {
			...
			array_push($Tipos_servicio,$fila);
		}
	...
	function Cargar_lineas_nuevas(){
		...
		$num_filas =$conexion->query("SELECT LINEAS_NUEVAS FROM hit_usuarios_formularios WHERE FORMULARIO = 'TIPOS_SERVICIO' AND USUARIO = '".$Usuario."'");
		if ($num_filas == FALSE){
	...
	function Cargar_combo_selector(){
		...
		$resultadoc =$conexion->query('SELECT * FROM hit_tipos_servicio'.$CADENA_BUSCAR);
	...
	function Modificar($codigo, $nombre, $venta_web){

		$conexion = $this ->Conexion;
		$query = "UPDATE hit_tipos_servicio SET ";
		$query .= " NOMBRE = '".$nombre."'";
		$query .= ", VENTA_WEB = '".$venta_web."'";
	...
	function Borrar($codigo){
		...
		$query = "DELETE FROM hit_tipos_servicio WHERE CODIGO = '".$codigo."'";
	...
	function Insertar($codigo, $nombre, $venta_web){

		$conexion = $this ->Conexion;
		$query = "INSERT INTO hit_tipos_servicio (CODIGO, NOMBRE, VENTA_WEB) VALUES (";
		$query .= "'".$codigo."',";
		$query .= "'".$nombre."',";
		$query .= "'".$venta_web."')";
	...
	function clsTipos_servicio($conexion, $filadesde, $usuario, $buscar_codigo, $buscar_nombre){
